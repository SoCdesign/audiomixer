...
-- Date:              Fri May 29 17:58:19 2015 (by Create and Import Peripheral Wizard)
...
architecture IMP of user_logic is
	...
	alias 	reset_in : STD_LOGIC is slv_reg0(0);
	signal OUT_mixer_ready_L : STD_LOGIC; --is slv_reg31(0);
	signal OUT_mixer_ready_R : STD_LOGIC; --is slv_reg31(1);

begin
	slv_reg31(0) <= OUT_mixer_ready_L;
	slv_reg31(1) <= OUT_mixer_ready_R;
	slv_reg31(C_SLV_DWIDTH - 1 downto 2) <= (others => '0');
...
--						end loop;
	...
	--	process(selected_channel, GM_Left_in_0, GM_Left_in_1, GM_Left_in_2, GM_Left_in_3, GM_Right_in_0, GM_Right_in_1, GM_Right_in_2, GM_Right_in_3)
	--	begin
	--		case(selected_channel) is
	--			when "00" => GM_Left_out <= GM_Left_in_0;
	--							 GM_Right_out <= GM_Right_in_0;
	--			when "01" => GM_Left_out <= GM_Left_in_1;
	--							 GM_Right_out <= GM_Right_in_1;
	--			when "10" => GM_Left_out <= GM_Left_in_2;
	...
	Mixer_inst_L : entity work.Mixer
		generic map(
			INTBIT_WIDTH  => 24,
			FRACBIT_WIDTH => 8
		)
		port map(
			...
			IN_CH3          => GM_Left_in_3,
			OUT_MIX         => GM_Left_out,
			OUT_mixer_ready => OUT_mixer_ready_L
		);
		
	Mixer_inst_R : entity work.Mixer
		generic map(
			INTBIT_WIDTH  => 24,
			FRACBIT_WIDTH => 8
		)
		port map(
			CLK             => CLK_48_in,
			RESET           => reset_in,
			IN_CH0          => GM_Right_in_0,
			IN_CH1          => GM_Right_in_1,
			IN_CH2          => GM_Right_in_2,
			IN_CH3          => GM_Right_in_3,
			OUT_MIX         => GM_Right_out,
			OUT_mixer_ready => OUT_mixer_ready_R
		);
	------------------------------------------
